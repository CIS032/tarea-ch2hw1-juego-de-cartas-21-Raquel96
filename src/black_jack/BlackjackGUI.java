...
//de datos con diálogos generados por la clase "javax.swing.JOptionPane
    public static void main(String[] args) {
        ...
        int apuestas = 0;        // Cantidad de apuestas de usuarios en un juego.
        boolean usuarioGano;     // ¿El usuario ganó el juego?
        String texto;

        dinero = 100;             // El usuario comieza el juego con $100.
        while (true) {
            texto = "\n "
                    + "Usted tiene: $"
                    + dinero
            ...
            do {
                texto = texto + "\n"
                        + "Cuánto dinero desea apostar?(Ingrese 0 para finalizar)\n?";
                apuestas = Integer.parseInt(JOptionPane.showInputDialog(null, texto,
                        "Bienvenido al juego de Blackjack", JOptionPane.INFORMATION_MESSAGE));
                if (apuestas < 0 || apuestas > dinero) {
                    JOptionPane.showMessageDialog(null, "\n Su respuesta debe estar entre 0 y "
                            + dinero + " ");
                }
    ...
    static boolean playBlackjack() {
       ...
       El jugador con Blackjack gana el juego. El distribuidor gana lazos.*/
        if (manoDistribuidor.getValorBlackJack() == 21) {
            aux = "\n El distribuidor tiene la " + manoDistribuidor.getCarta(0)
                    + " y la " + manoDistribuidor.getCarta(1) + "."
                    + "\n El usuario tiene la " + manoUsuario.getCarta(0)
        ...
        if (manoUsuario.getValorBlackJack() == 21) {
            aux = "\n El distribuidor tiene la " + manoDistribuidor.getCarta(0) + " y la "
                    + manoDistribuidor.getCarta(1) + "." + "\n El usuario tiene la "
                    + manoUsuario.getCarta(0) + " y la" + manoUsuario.getCarta(1) + " . "
...
//Mostrar tarjetas de usuario, y dejar que el usuario decida golpear o pararse. 
            ...
            do {
                String car = (JOptionPane.showInputDialog(null, "Jugar (G) o Levantarse (L) ?",
                        "Ingrese una opción", JOptionPane.INFORMATION_MESSAGE));
                ...
                if (accionUsuario != 'G' && accionUsuario != 'L') {
                    JOptionPane.showMessageDialog(null, "\nPor favor responda G o L:  "
                            + "", "Elección equivocada", JOptionPane.INFORMATION_MESSAGE);
                }
            ...
            if (accionUsuario == 'L') {
                ...
                if (manoUsuario.getValorBlackJack() > 21) {
                    ...
                    JOptionPane.showMessageDialog(null, aux, "Has fallado al pasar de 21. Usted Pierde",
        ...
        if (manoDistribuidor.getValorBlackJack() == manoUsuario.getValorBlackJack()) {
            ...
            aux = "\n El distribuidor gana " + manoDistribuidor.getValorBlackJack() + " tu puntuacion "
                    + manoUsuario.getValorBlackJack() + " . ";
            JOptionPane.showMessageDialog(null, aux, "Jack Black", JOptionPane.INFORMATION_MESSAGE);
            return false;
        } else {
            aux = "\n Usted gana " + manoUsuario.getValorBlackJack() + " Puntuación del ganador "
                    + manoDistribuidor + " . ";
            JOptionPane.showMessageDialog(null, aux, "Jack Black", JOptionPane.INFORMATION_MESSAGE);
            return true;
        }
